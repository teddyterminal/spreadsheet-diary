...
def compute(df):
    ...
    df["wins_30"] = df["win_score"].rolling(30).sum()

    # Year columns
    df["year"] = df["date"].dt.year

    df["ytd_w"] = df.groupby("year")["diff"].apply(lambda s: (s > 0).cumsum())
    df["ytd_l"] = df.groupby("year")["diff"].apply(lambda s: (s < 0).cumsum())
    df["ytd_t"] = df.groupby("year")["diff"].apply(lambda s: (s == 0).cumsum())

    df["ytd_pw"] = df.groupby("year")["diff"].apply(lambda s: s.clip(lower=0).cumsum())
    df["ytd_pl"] = df.groupby("year")["diff"].apply(lambda s: s.clip(upper=0).cumsum())
    df["ytd_pt"] = df.groupby("year")["diff"].apply(lambda s: (s == 0).astype(int).cumsum())
